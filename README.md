1) Перед тем как запускать контейнер, убедитесь, что у вас установлен и запущен docker и git  
2) Клонируйте необходимую ветку репозитория командой ```git clone git@github.com:Lynatuk666/dockerhomework.git -b master ```   
3) Соберите образ командой ```docker build -t image-name .  ```  
4) Запустите контейнер командой ```docker run -d -p 8000:8000 image-name```   
5) Наслаждайтесь, вы прекрасны, сайт работает и никто даже не умер(надеюсь)    
